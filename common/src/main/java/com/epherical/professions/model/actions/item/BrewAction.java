...
public class BrewAction extends AbstractItemAction {
    ...
    public static final MapCodec<BrewAction> CODEC = RecordCodecBuilder.mapCodec(
            i -> i.group(
                    ...
                    Action.tagOrElementListCodec(Registries.ITEM).fieldOf("items").forGetter(BrewAction::getValues)
            ).apply(i, BrewAction::new));


    public BrewAction(Common common, List<Either<TagKey<Item>, ResourceKey<Item>>> targets) {
    ...
    public static class Builder extends Action.Builder<Builder, Item> {
        ...
        public Action<Item> build() {
            return new BrewAction(new Common(getProfession(), getConditions(), getRewards()), getTargets());
        }
    }
}
