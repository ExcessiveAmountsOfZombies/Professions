...
public class TameAction extends AbstractEntityAction {
    ...
    public static final MapCodec<TameAction> CODEC = RecordCodecBuilder.mapCodec(
            i -> i.group(
                    COMMON.forGetter(TameAction::buildCommon),
                    Action.tagOrElementListCodec(Registries.ENTITY_TYPE).fieldOf("entities").forGetter(TameAction::getValues)
            ).apply(i, TameAction::new));

    protected TameAction(Common common, List<Either<TagKey<EntityType<?>>, ResourceKey<EntityType<?>>>> targets) {
    ...
    public static class Builder extends Action.Builder<Builder, EntityType<?>> {
        ...
        public Action<EntityType<?>> build() {
            return new TameAction(new Common(getProfession(), getConditions(), getRewards()), getTargets());
        }
    }
}
