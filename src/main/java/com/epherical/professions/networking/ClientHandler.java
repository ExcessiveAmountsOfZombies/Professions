...
public class ClientHandler {
    ...
    private static final Map<ResourceLocation, ClientPlayNetworking.PlayChannelHandler> registerSubChannelReceivers = new HashMap<>();
    private static final Map<CommandButtons, CommandButtonHandler> buttonHandlers = new HashMap<>();
    ...
    public static void receivePacket(Minecraft minecraft, ClientPacketListener listener, FriendlyByteBuf buf, PacketSender sender) {
        ...
        ClientPlayNetworking.PlayChannelHandler handler = registerSubChannelReceivers.getOrDefault(subChannel, null);
        if (handler != null) {
    ...
    private static void setupSubChannels() {
        registerSubChannelReceivers.put(PacketIdentifiers.OPEN_UI_RESPONSE, (client, handler, buf, responseSender) -> {
        ...
        registerSubChannelReceivers.put(PacketIdentifiers.CLICK_PROFESSION_BUTTON_RESPONSE, (client, handler, buf, responseSender) -> {
            CommandButtons button = buf.readEnum(CommandButtons.class);
            CommandButtonHandler buttonHandler = buttonHandlers.getOrDefault(button, null);
    ...
    private static void setupButtonHandlers() {
        buttonHandlers.put(CommandButtons.JOIN, (minecraft, buf) -> {
