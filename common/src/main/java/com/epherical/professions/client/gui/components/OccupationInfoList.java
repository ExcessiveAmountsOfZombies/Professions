...
public class OccupationInfoList extends ContainerObjectSelectionList<OccupationInfoList.Entry> {
    ...
    public OccupationInfoList(Minecraft mc, int width, int top, int bottom, int height) {
        ...
        Collection<Action<?>> actionsByProfession = CommonClass.ACTION_LOAD2.getActionManager().getActionsByProfession(this.profession);
        List<EntryItem> items = new ArrayList<>();
        for (Action<?> action : actionsByProfession) {
            Collection<Holder<?>> actionsByValue = CommonClass.ACTION_LOAD2.getActionManager().getValuesForAction(action);
